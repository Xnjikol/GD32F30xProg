...
int flux_observer_init(flux_observer_t* observer, const flux_observer_params_t* params)
{
  ...
  if (params->Rs <= 0.0f || params->Ls <= 0.0f || params->Ts <= 0.0f || params->flux_rated <= 0.0f)
  {
    return -1;
  }
                           ...
                           Clark_t* current)
{
  ...
  {
    observer->lpf_alpha = (LowPassFilter_t*) calloc(1, sizeof(LowPassFilter_t));
  }
  if (observer->lpf_beta == NULL)
  {
    observer->lpf_beta = (LowPassFilter_t*) calloc(1, sizeof(LowPassFilter_t));
  }


  // observer->flux_alpha_lpf = observer->flux_alpha_lpf * (1.0f - observer->lpf_coeff) +
  //                            observer->flux_alpha * observer->lpf_coeff;
  // observer->flux_beta_lpf = observer->flux_beta_lpf * (1.0f - observer->lpf_coeff) +
  //                           observer->flux_beta * observer->lpf_coeff;

  /* 去除直流分量 */
  observer->flux->a = LowPassFilter_Update(observer->lpf_alpha, observer->flux->a);
  observer->flux->b = LowPassFilter_Update(observer->lpf_beta, observer->flux->b);
