...
static inline bool  Estimate_Rs(float  Current,
                                float* Voltage_out,
                                float* Rs);
static LLS_Result_t Single_Axis_LLS(FluxExperiment_t* exp,
                                    int               exponent);
static void process_cycle_for_dq_adq(FluxExperiment_t* exp, int s);
static void Compute_SingleAxis_SSR_R2(FluxExperiment_t* exp,
                                      int               exponent,
                                      int               axis);

/* 结果结构：每个 Imax 只保存最终的 avg_max_psi（和 Imax 值） */

void Get_Identification_Results(FluxExperiment_t* exp,
                                float*            ad0,
                                float*            add,
                                float*            aq0,
                                float*            aqq,
...
/* ---------- 初始化 ---------- */
void Experiment_Init(FluxExperiment_t* exp,
                     float             Ts,
                     int               sample_capacity,
                     int               repeat_times,
                     int               max_steps,
...
{
    // clip params
    if (sample_capacity > SAMPLE_CAPACITY)
    {
...
void Experiment_Step(
    FluxExperiment_t* exp, float Id, float Iq, float* Ud, float* Uq)
{
    if (!exp)
        return;
    ...
    case INJECT_COLLECT:
    {
        ...
        if (exp->inj.State)
        {
            ...
            if (exp->inj.mode == INJECT_D || exp->inj.mode == INJECT_DQ)
            {
                ...
                exp->inj.Vd = (exp->inj.inj_state_d >= 0)
                                  ? exp->inj.Ud_amp
                                  : -exp->inj.Ud_amp;
            }
            ...
            if (exp->inj.mode == INJECT_Q || exp->inj.mode == INJECT_DQ)
            {
                ...
                exp->inj.Vq = (exp->inj.inj_state_q >= 0)
                                  ? exp->inj.Uq_amp
                                  : -exp->inj.Uq_amp;
            }
        ...
        if (exp->edge_count >= exp->wait_edges + 1)
        {
            if (exp->pos < exp->sample_capacity)
            {
                ...
                else
                {
                    int   prev = idx - 1;
                    float integrand_d
                        = (exp->last_Vd - exp->Rs_est * Id);
                    float integrand_q
                        = (exp->last_Vq - exp->Rs_est * Iq);
                    exp->psi_d_buf[idx]
                        = exp->psi_d_buf[prev] + exp->Ts * integrand_d;
                    exp->psi_q_buf[idx]
                        = exp->psi_q_buf[prev] + exp->Ts * integrand_q;
                }
                exp->pos++;
    ...
    case PROCESS:
    {
        ...
        if (exp->inj.mode == INJECT_D || exp->inj.mode == INJECT_Q)
        {
            ...
            float* psi_buf = (exp->inj.mode == INJECT_Q)
                                 ? exp->psi_q_buf
                                 : exp->psi_d_buf;
            ...
            else
            {
                // 达到重复次数：计算平均并保存结果
                float avg_psi
                    = exp->sum_max_psi / (float)exp->repeat_times;
                float avg_I = exp->sum_max_I / (float)exp->repeat_times;

                exp->results[exp->step_index].Imax_value
                    = avg_I;  // 现在是峰值点对应的电流
                exp->results[exp->step_index].avg_max_psi
                    = avg_psi;  // 峰值点磁链
                exp->results[exp->step_index].cycles_used
                    = exp->repeat_times;
    ...
    case LLS:
    {
        ...
        if (exp->inj.mode == INJECT_D)
        {
            ...
            exp->LLS.ad0 = lls.ad0;
            exp->LLS.add = lls.add;
            Compute_SingleAxis_SSR_R2(
                exp, X, 0);  // 计算 D 轴的 SSR 和 R^2
        }
        if (exp->inj.mode == INJECT_Q)
        {
            X            = 1;
        ...
        if (exp->inj.mode == INJECT_DQ)
        {
            ...
            float SST_id
                = exp->sum_id2
                  - (exp->sum_id * exp->sum_id)
                        / (float)exp->count_id;  // sum (id - mean)^2
            float R2_d = (SST_id > 0.0f) ? (1.0f - Jd / SST_id) : 0.0f;

            float SST_iq
                = exp->sum_iq2
                  - (exp->sum_iq * exp->sum_iq) / (float)exp->count_iq;
...
static bool Estimate_Rs(float Current, float* Voltage_out, float* Rs)
{
    ...
    I_filtered
        = CURRENT_FILTER_ALPHA * Current
          + (1.0F - CURRENT_FILTER_ALPHA)
                * I_filtered;  // 如果已完成，直接输出V=0并返回true
    if (done_flag)
    {
        V_now        = 0.0F;
        *Voltage_out = V_now;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                if ((fabsf(Rs_new - Rs_last) < RS_THRESHOLD
                     && est_step > 0)
                    && (fabsf(I_filtered)
                        >= CURRENT_LIMIT * CURRENT_RATIO))
                {
                    *Rs       = Rs_new;
                    first     = true;
                    est_step  = 0;
...
void process_cycle_for_dq_adq(FluxExperiment_t* exp, int s)
{
    for (int i = exp->edge_idx[0]; i < exp->edge_idx[1]; ++i)
    {
        ...
        if (!(id_s > 0.0F && iq_s > 0.0F && psi_d > 0.0F
              && psi_q > 0.0F))
            continue;
        ...
        float psi_d_S1 = 1.0F;
        for (int k = 0; k < s + 1; ++k)
            psi_d_S1 *= psi_d;  // psi_d^(S+1)
        float psi_q_T1 = 1.0F;
        for (int k = 0; k < Tn + 1; ++k)
            psi_q_T1 *= psi_q;  // psi_q^(T+1)
        ...
        float psi_d_U1 = 1.0F;
        for (int k = 0; k < U + 1; ++k)
            psi_d_U1 *= psi_d;              // psi_d^(U+1)
        float psi_d_U2 = psi_d_U1 * psi_d;  // psi_d^(U+2)
        float psi_q_V1 = 1.0F;
        for (int k = 0; k < V + 1; ++k)
            psi_q_V1 *= psi_q;              // psi_q^(V+1)
                                      ...
                                      int               axis)
{
    ...
    double sumI = 0.0;
    for (int i = 0; i < N; ++i)
        sumI += (double)exp->results[i].Imax_value;
    ...
    for (int i = 0; i < N; ++i)
    {
        ...
        if (axis == 0)
        {
            I_pred = (double)exp->LLS.ad0 * psi
                     + (double)exp->LLS.add * xp;
        }
        else
        {
            I_pred = (double)exp->LLS.aq0 * psi
                     + (double)exp->LLS.aqq * xp;
        }
