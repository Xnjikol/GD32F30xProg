...
#    error "请在foc.h中定义GATE_POLARITY_HIGH_ACTIVE或GATE_POLARITY_LOW_ACTIVE"
#endif
...
#define MAIN_INT_TIMER_PERIOD    12000 /* 周期值：12000 (5kHz = 120MHz/12000/2) */
#define MAIN_INT_TIMER_DEADTIME_PERIOD \
    2000 /* 死区时间：2us (2us = 2000/120MHz/12000/2) */

/* 主循环(电流环)频率配置 */
#define MAIN_LOOP_FREQ                                                         \
    (MCU_MAIN_FREQ / (MAIN_INT_TIMER_PRESCALER + 1.0F) / MAIN_INT_TIMER_PERIOD \
     / 2)                                      /* 5kHz */
#define MAIN_LOOP_TIME (1.0F / MAIN_LOOP_FREQ) /* 200us */

/* 转速环频率配置 */
#define SPEED_LOOP_PRESCALER 10.0F /* 转速环分频系数 */
#define SPEED_LOOP_FREQ      (MAIN_LOOP_FREQ / SPEED_LOOP_PRESCALER) /* 500Hz */
...
#define PID_SPEED_LOOP_INTEGRAL_LIMIT PID_SPEED_LOOP_MAX_OUTPUT /* 积分限幅值 */
#define PID_SPEED_LOOP_TIME           (SPEED_LOOP_TIME) /* 转速环采样周期 */

#define CURRENT_LOOP_WC 300.0F /* 电流环带宽 (rad/s) */

/* 电流环d轴PID参数配置 */
#define PID_CURRENT_D_LOOP_KP MOTOR_LD* CURRENT_LOOP_WC /* d轴比例系数 */

#define PID_CURRENT_D_LOOP_KI MOTOR_RS* CURRENT_LOOP_WC /* d轴积分系数 */

#define PID_CURRENT_D_LOOP_KD 0.00F /* d轴微分系数 */

/* d轴输出限制 */
#define PID_CURRENT_D_LOOP_MAX_OUTPUT 200.0F /* 最大输出电压：Udc/√3 */
#define PID_CURRENT_D_LOOP_MIN_OUTPUT (-1.0F * PID_CURRENT_D_LOOP_MAX_OUTPUT)
...
/* 电流环q轴PID参数配置 */
#define PID_CURRENT_Q_LOOP_KP MOTOR_LQ* CURRENT_LOOP_WC /* q轴比例系数 */

#define PID_CURRENT_Q_LOOP_KI MOTOR_RS* CURRENT_LOOP_WC /* q轴积分系数 */

#define PID_CURRENT_Q_LOOP_KD 0.00F /* q轴微分系数 */

/* q轴输出限制 */
#define PID_CURRENT_Q_LOOP_MAX_OUTPUT 350.0F /* 最大输出电压：Udc/√3 */
#define PID_CURRENT_Q_LOOP_MIN_OUTPUT (-1.0F * PID_CURRENT_Q_LOOP_MAX_OUTPUT)
...
/*                         带速重投参数配置                            */
/*********************************************************************/
#define SHORT_COUNT   3U  /* 短接采样点数 */
#define RELEASE_COUNT 8U  /* 开路采样点数 */
#define DELTA_COUNT   12U /* 第二次开路增加点数 */
