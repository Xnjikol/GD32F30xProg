...
/*--------------------------------------------------------------------------*/
/* Get Address from CAN frame */
/* !!! Designed for standard CAN frame Only !!! */
/*--------------------------------------------------------------------------*/

CCP_MTABYTEPTR ccpGetPointer(CCP_BYTE addr_ext, CCP_DWORD addr)        // get Pointer into
{ 
  (void)addr_ext;                                                          // normal C
...
/* double conversion */
  ...
  //  }
  {
    ...
    // 将数据逆序写入内存，使得高字节先存到低地址
    for (int i = 0; i < size; i++)
    {
      *(ccp.MTA[n]) = temp[size - 1 - i];
      ccp.MTA[n]++;
    }
  }
  return CCP_WRITE_OK;
  ...
  while (d < dl && e < el && e->ptr)
  {
    ...
    if (s == 8)
    {
      *(float *)d = (float)(*(double *)p);
      ...
      //   *d = *p;
      // }
      /*更换DAQ采样时的字节序，因为Pisnoop是按照低字节在前解析的DAQ信息*/
      if (s == 4)
      {
        CCP_DWORD val = *(CCP_DWORD *)p;
        *(CCP_DWORD *)d =
            ((val & 0x000000FFUL) << 24) |
            ((val & 0x0000FF00UL) << 8) |
            ((val & 0x00FF0000UL) >> 8) |
      ...
      {
        CCP_WORD w = *(CCP_WORD *)p;
        *(CCP_WORD *)d = ((w & 0x00FFU) << 8) | ((w & 0xFF00U) >> 8);
      }
      else
      {
        *d = *p;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: EXCANGE_ID master_id=%u\n", ctr, masterId);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: GET_SEED resource=%02X, protection_status=%u\n", ctr, privilegeLevel, ccp.Crm[3]);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: UNLOCK protection_status=%02X\n", ctr, ccp.ProtectionStatus);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: SET_MTA %u to %08lX,%u\n", ctr, mta, addr, addrExt);
#endif
...
#ifdef CCP_MOTOROLA
#define daqId (*(CCP_DWORD *)&com[4]) /* Has to be Motorola-Format ! */
#endif
#ifdef CCP_INTEL
#define daqId (((CCP_DWORD)com[4] << 24) | ((CCP_DWORD)com[5] << 16) | ((CCP_DWORD)com[6] << 8) | ((CCP_DWORD)com[7]))
#endif
#ifdef CCP_DAQ
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: GET_DAQ_SIZE daq=%u,id=%lu,size=%u,pid=%u\n", ctr, daqList, daqId, ccp.Crm[3], daqList * CCP_MAX_ODT);
#endif
#else
      ccp.Crm[3] = 0;
      ccp.Crm[4] = 0;
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: GET_DAQ_SIZE daq=%u,size=0\n", ctr, daqList, daqId);
#endif
#endif
#undef daqList
#undef daqId
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: SET_DAQ_PTR daq=%u,odt=%u,idx=%u\n", ctr, comDaq, comOdt, comIdx);
#endif
...
#endif
#ifdef CCP_INTEL
#define writeDaqAddr (((CCP_DWORD)com[4] << 24) | ((CCP_DWORD)com[5] << 16) | ((CCP_DWORD)com[6] << 8) | ((CCP_DWORD)com[7]))
#endif
      if (
#ifdef CCP_ODT_ENTRY_SIZE
#ifdef CCP_DOUBLE_FLOAT
          (((writeDaqSize != 8) && (writeDaqSize != 1 && writeDaqSize != 2 && writeDaqSize != 4)))
#else
...
#endif
#ifdef CCP_INTEL
#define ssPrescaler (((CCP_WORD)com[6] << 8) | ((CCP_WORD)com[7])) /* Prescaler */
#endif
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: START_STOP cmd=%u,daq=%u,last=%u,eventChannel=%u,prescaler=%u\n", ctr, ssCmd, ssDaq, ssLast, ssEventChannel, ssPrescaler);
#endif
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: START_STOP_ALL cmd=%u\n", ctr, ssCmd);
#endif
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: BUILD_CHKSUM size=%u\n", ctr, s);
#endif
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: CLEAR_MEMORY size=%u\n", ctr, s);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: SEL_CAL_PAGE id=%08Xh\n", ctr, (CCP_DWORD)ccp.MTA[0]);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: GET_CAL_PAGE -> id=%08Xh\n", ctr, *(CCP_DWORD *)&ccp.Crm[4]);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: SET_S_STATUS status=%02X\n", ctr, ccp.UserSessionStatus);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: GET_S_STATUS -> status=%02X\n", ctr, ccp.Crm[3]);
#endif
    }
    break;
...
#ifdef CCP_TESTMODE
      if (gDebugLevel)
        CCPPRINT("%u: GET_VERSION -> major=%02X minor=%02X\n", ctr, ccp.Crm[3], ccp.Crm[4]);
#endif
    }
    break;
  ...
  for (j = 0; j < CCP_MAX_ODT; j++)
  {
    for (i = 0, f = 1; i <= 6; i++)
    {
      if (ccp.DaqList[daq].odt[j][i].ptr)
      {
        if (f)
        {
          CCPPRINT(" PID %u: ", daq * CCP_MAX_ODT + j);
          f = 0;
        }
#ifdef CCP_ODT_ENTRY_SIZE
